...
The first structure that we will learn to generate today serves as an excellent example of a higher period transition metal complex with a vital application in medicinal inorganic chemistry. It’s an Osmium(VI) nitrido compound with the formula OsN(2,2′-bipyridine)Cl<sub>3</sub> that exhibits anticancer properties.
...
`molsimplify legacy –i input.in`


Alternatively, you can also generate the structure by specifying minimal arguments in the command line interface as:


`molsimplify legacy -core Os -lig bipy chloride '[N]' chloride -ligocc 1 1 1 2 -smicat '[1]' -oxstate 6` 


The .xyz file for the structure will be generated and saved in the Runs directory of molSimplify. The final structure for OsN(2,2′-bipyridine)Cl<sub>3</sub> generated by molSimplify is shown below.
...
Let’s generate a simple molecule like Zr(H<sub>2</sub>O)<sub>8</sub> with a square anti-prismatic geometry. The steps for generating the structure are the same as those for the Osmium compound. In fact, the input arguments are even simpler. You can type the following command in the command line interface to generate an .xyz file for Zr(H<sub>2</sub>O)<sub>8</sub> :


`molsimplify legacy -core Zr -coord 8 -lig water -ligocc 8` 


Similarly, we can also generate a 7 coordinate complex such Ru(Cl)<sub>7</sub> by the command:


`molsimplify legacy -core Ru -coord 7 -lig chloride -ligocc 7` 


The structures for Zr(H<sub>2</sub>O)<sub>8</sub> and Ru(Cl)<sub>7</sub> generated by these commands are shown below.
