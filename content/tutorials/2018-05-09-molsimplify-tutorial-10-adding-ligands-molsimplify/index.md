...
You can see what ligands are in the molSimplify Ligand set using the command `molsimplify -h liganddict`. You can also make your own ligands and add them to the Ligand set in molSimplify/Ligands (need to add both a structure file and an additional line in `ligands.dict` file). As indicated above, another option for custom ligands is to input a SMILES string after `-lig` during complex generation. 
